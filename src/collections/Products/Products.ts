...
const isAdminOrHasAccess = (): Access => ({ req: { user: _user } }) => {
    ...
    const userProductsId = (user?.products || []).reduce<Array<string>>((acc, product) => {
        ...
        beforeChange: [
            addUser, async (args) => {
                if (args.operation === 'create') {
                    ...
                    const udpatedProduct = await stripe.products.update(data?.stripeId!, {
                        name: data?.name,
                        default_price: data?.priceId!
                    })
                    const updated: Product = {
                        ...data,
                        stripeId: udpatedProduct.id,
                        priceId: udpatedProduct.default_price as string
                    }
                    return updated
                }
            }
